@php
    ...
    $faqs = [
        [
            ...
            'a' =>
                'Start a live chat from the widget in the bottom-right of any page, or email support@helix.io. Pro and Business plans get priority response times.',
        ],
        [
            'q' => 'Where can I see my invoices?',
            'a' =>
                'Open Settings → Billing. Every invoice is downloadable as a PDF, and you can update your billing email there too.',
        ],
        [
            'q' => 'Is there a status page?',
            'a' =>
                'Yes — status.helix.io shows real-time uptime and incident history. Subscribe to get notified of any disruptions.',
        ],
    ];
@endphp
    ...
    <section class="relative overflow-hidden border-b">
        ...
        <div class="mx-auto max-w-2xl px-6 py-20 text-center">
            ...
            <div class="mx-auto mt-7 max-w-xl">
                <x-ui.input type="search" placeholder="Search for articles, e.g. “reset password”…"
            ...
            <div class="text-muted-foreground mt-4 flex flex-wrap items-center justify-center gap-2 text-sm">
                ...
                @foreach (['Billing', 'SSO', 'API keys', 'Export data'] as $p)
                    <a href="#"
                        class="bg-muted hover:bg-accent rounded-full px-3 py-1 transition-colors">{{ $p }}</a>
                @endforeach
            </div>
        </div>
    </section>
        ...
        <section class="py-16">
            ...
            <div class="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
                @foreach ($categories as $c)
                    <a href="#"
                        class="group bg-card hover:border-ring/60 rounded-xl border p-6 shadow-sm transition-all hover:-translate-y-0.5 hover:shadow-md">
                        <div
                            class="bg-primary/10 text-primary mb-4 inline-flex size-11 items-center justify-center rounded-lg">
                            <x-dynamic-component :component="'lucide-' . $c['icon']" class="size-5" />
                        </div>
                        <h3 class="group-hover:text-primary flex items-center gap-1.5 font-semibold transition-colors">
                            {{ $c['title'] }} <x-lucide-arrow-right
                                class="size-4 opacity-0 transition-opacity group-hover:opacity-100" /></h3>
            ...
            <div>
                ...
                <ul class="divide-y rounded-xl border">
                    @foreach ($popular as $a)
                        <li><a href="#"
                                class="hover:bg-accent/50 flex items-center justify-between gap-3 px-4 py-3.5 text-sm font-medium transition-colors"><span
                                    class="flex items-center gap-2.5"><x-lucide-file-text
                                        class="text-muted-foreground size-4" /> {{ $a }}</span>
                                <x-lucide-chevron-right class="text-muted-foreground size-4" /></a></li>
                    @endforeach
                </ul>
            </div>
            <div>
                ...
                <x-ui.accordion type="single" collapsible class="w-full">
                    @foreach ($faqs as $i => $faq)
                        <x-ui.accordion-item
                            value="faq-{{ $i }}"><x-ui.accordion-trigger>{{ $faq['q'] }}</x-ui.accordion-trigger><x-ui.accordion-content>{{ $faq['a'] }}</x-ui.accordion-content></x-ui.accordion-item>
                    @endforeach
                </x-ui.accordion>
            </div>
        </section>
            ...
            <div class="mt-8 grid gap-5 sm:grid-cols-3">
                @foreach ([['message-circle', 'Live chat', 'Replies in ~2 min', 'Start chat'], ['mail', 'Email us', 'support@helix.io', 'Send email'], ['users', 'Community', '12k+ members', 'Visit forum']] as [$ic, $t, $d, $cta])
                    <x-ui.card class="flex flex-col items-center text-center">
                        <div
                            class="bg-primary/10 text-primary mb-3 inline-flex size-12 items-center justify-center rounded-full">
                            <x-dynamic-component :component="'lucide-' . $ic" class="size-5" />
                        </div>
